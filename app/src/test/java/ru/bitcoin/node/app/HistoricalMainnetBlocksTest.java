...
class HistoricalMainnetBlocksTest {
    @TempDir Path directory;
    @Test void importsFirstThreeMainnetBlocksAndRestoresCoinsAfterRestart() throws Exception {
        List<OutPoint> outputs = new ArrayList<>();
        String[] manifest;
        try(var in = Objects.requireNonNull(getClass().getResourceAsStream("/mainnet/blocks.txt"))) {
        ...
        try(var db = new RocksDbDatabase(directory)) {
            var service = new NodeValidationService(db,NetworkParametersRegistry.mainnet(),()->1_800_000_000L,new Mempool());
            for(String line:manifest) {
                var fields=line.split(" ");
                int height=Integer.parseInt(fields[0]);
                try(var in=Objects.requireNonNull(getClass().getResourceAsStream("/mainnet/"+height+".bin"))) {
                    ...
                    assertArrayEquals(tx.outputs().getFirst().scriptPubKey(),coin.scriptPubKey());
                }
            }
        }
        try(var db=new RocksDbDatabase(directory)) {

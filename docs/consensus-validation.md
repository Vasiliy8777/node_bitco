...
Актуальное описание сетевого и RPC-майнингового контура: [mining-node.md](mining-node.md).
Ниже сохранена история этапов; ранние замечания об отсутствии обработчиков относятся
к состоянию соответствующего этапа.
...
- Проверен импорт блока 1 публичного Signet; полный исторический прогон Signet не выполнен.
...
## Начало реализации mining: coinbase

`CoinbaseBuilder.build` формирует BIP34 height prefix, extraNonce в пределах 100 байт,
выплату subsidy + проверенная вызывающим кодом комиссия и BIP141 witness commitment
по выбранным транзакциям. Reserved value фиксирован нулями. До SegWit witness не
создаётся, выборка с witness отклоняется. Проверяются диапазон суммы и отсутствие
другой coinbase. Тесты покрывают границы кодирования высоты, халвинг, basic/BIP34
валидацию и привязку commitment к выборке. Сборщик шаблона и цикл майнинга ещё не готовы.

Правило исключения непотратимых выходов объединено в `UnspendableScript` для всех
трёх путей подключения/отключения. Добавлены проверки размера 10000/10001, начального
OP_RETURN и его байта внутри push-данных.

## Контекстно проверяемый кандидат блока

BlockTemplateBuilder принимает упорядоченную выборку транзакций и стабильный контекст
цепи, вычисляет комиссии через временный UTXO overlay, строит coinbase/Merkle root
и запускает BlockConnectChangesBuilder без записи изменений. Тест проверяет комиссию,
отсутствие записи в исходный store и отказ при двойной трате. Версия заголовка, время
и bits пока поступают от вызывающего кода; автоматический отбор, выбор сложности,
обновление шаблона и поиск nonce ещё не реализованы. Возвращается кандидат без PoW,
а не готовый к публикации блок. Вызывающий код должен удерживать блокировку цепи.

## Ограниченный поиск nonce

NonceMiner.search перебирает заданный диапазон uint32 без переполнения, проверяет
целевой target, поддерживает отмену и прерывание потока. Возвращает Optional<Block>,
не меняя транзакции и Merkle root. Тесты проверяют известный genesis nonce,
границы диапазона, отмену и новый regtest-кандидат из BlockTemplateBuilder.
Найденный блок ещё должен проходить актуальный BlockProcessor. Автоматическая
смена extraNonce, обновление шаблона, выбор bits и публикация пока не реализованы.

## Наш блок принят независимым Core

MinedBlockAdmissionTest собирает и добывает regtest-блок высоты 1 нашим кодом,
принимает его через NodeValidationService/BlockProcessor и проверяет монету и перезапуск.
Точный сериализованный блок отдельно принят свежим Bitcoin Core v31.1.0 через
submitblock; getbestblockhash совпал с локальным. Блок и ответ RPC сохранены в
app/src/test/resources/local-mining. Внешний запуск пока ручной, Maven выполняет
локальную часть. Блок пустой: отбор mempool-транзакций и автоматический цикл ещё нужны.

## Отбор транзакций

TransactionSelector.select выбирает ancestor-пакеты по суммарной комиссии/vsize,
выдаёт родителей перед детьми, пересчитывает стоимость после выбора предков и
соблюдает отдельные бюджеты weight/sigops и минимальную комиссию. Вход — уже
валидированный бесконфликтный снимок mempool. Бюджеты должны заранее исключать
coinbase, заголовок и накладные расходы сериализации; итог проходит BlockTemplateBuilder.
Тесты покрывают CPFP, порядок снимка, ограничения и однократный учёт родителя.
Это простой greedy-алгоритм без оптимизации для большого пула; он пока не подключён
к автоматическому обновлению шаблонов в NodeValidationService.

## Селектор подключён к сборщику

BlockTemplateBuilder.fromMempool резервирует вес coinbase, заголовка и максимальную
длину CompactSize счётчика транзакций, а также legacy sigops coinbase. Остальной
бюджет передаётся селектору; итог проходит повторную контекстную валидацию и проверку
заданного веса. Тесты покрывают выбор транзакции, комиссию и нехватку бюджета.
Резерв CompactSize консервативен. API всё ещё требует стабильного снимка под
блокировкой цепи и заданных version/time/bits; автоматический сервис не подключён.

## Создание шаблона через сервис

NodeValidationService.createMiningTemplate синхронизирует mempool и строит кандидат
под общей блокировкой цепи. Время выбирается max(adjustedTime, MTP+1), bits вычисляются
общим ChainHeaderValidator.nextBits. Заголовок использует versionbits top mask без
сигнализации отдельных deployment. Каждый вызов строит свежий кандидат. Тест
MiningTemplateServiceTest проверяет PoW/приём первого блока и обновление родителя и
coinbase следующего шаблона. Это API по запросу, не фоновый цикл; отмена устаревшего
поиска, RPC, уведомления майнера и отдельная поддержка Signet-подписания ещё нужны.

## Публичный Signet

PublicSignetBlockTest проверяет сохранённый блок 1 публичного Signet: закреплённый
хеш, default challenge, полное подключение через сервис и восстановление вершины
после чистого перезапуска. Изменение подписанного timestamp отклоняется прямой
проверкой challenge независимо от PoW. Оба теста прошли; изменений алгоритма
SignetBlockValidator для этого примера не потребовалось. Это не возможность майнить
публичный Signet: для выпуска блока необходима подпись разрешённого challenge.

Дополнительно PublicSignetBlockTest проверяет привязку подписи к coinbase: изменение
выплаты с пересчитанным Merkle root сохраняет корректность структуры и witness
commitment, но отклоняется Signet challenge. Отдельно проверено, что изменение nonce
не нарушает подпись challenge: PoW проверяется независимо после подписания.
